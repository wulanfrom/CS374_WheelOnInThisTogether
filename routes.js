$( document ).ready(function() {
  ...
  function loadCard(id, imgl, title, username, likes, descl, ramp, ele, wheel){
    var card = "<div class='card route-card' id='" + id + "'><div class='route-card-image' style='background-image: url('" + imgl[0] + "');'></div>"
                ...
                + username + "</p></div></div><div class='route-likes'><span class=icon-like aria-hidden='true' id='heart-number'></span>"
                + "<small class='number-of-likes' id='route-number-likes'>" + likes + "</small></div><div class='route-card-contents'>"
    ...
    card = card + "</div><div class='route-card-blue'></div><a href='#' onclick='movePage(this);'>"
  ...
  function fillPage(id){
    ...
    query.once('value').then(function(snapshot){
       snapshot.forEach(function(childSnapshot){
         ...
         contentList = [title_db, username_db, likes_db, descriptionList, landmarks_db, imageList];
       });
  ...
  function movePage(element){
    ...
    window.location = "routes-individual.html";
    fillPage(id);
  }
})
